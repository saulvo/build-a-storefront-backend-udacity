...
export class UserModel {
  ...
  async create(user: IUserCreate): Promise<IUser> {
    ...
    try {
      ...
      const hash = bcrypt.hashSync(password, parseInt(process.env.SALT_ROUNDS as string, 10));
  ...
  async update(id: string, user: IUser): Promise<IUser> {
    try {
      const sqlQuery = 'UPDATE users SET firstname = $1, lastname = $2, username = $3, password = $4 WHERE id = $5 RETURNING *';
      const connect = await db.connect();
      const result = await connect.query(sqlQuery, [user.firstname, user.lastname, user.username, user.password, id]);
      connect.release();
      return result.rows[0];
    } catch (error) {
